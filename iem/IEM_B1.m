...
load('/Volumes/Passport/ContLTM_IEM/101/Session_1/MPRAGE/S2_mydata_obj.mat');
...
figure
hold off; 
plot(mydata.condsX,'o'); 
xlabel('Trial'); ylabel('Color (deg)'); title('Color Labels');

stim_mask = zeros(size(mydata.condsX,1),length(xrad));
for tt = 1:size(stim_mask,1)  % loop over trials
    if ~isnan(mydata.condsX(tt))
        stim_mask(tt,mydata.condsX(tt))=1;
    end
end
...
% ylim([.15 round(max(mean(chan_resp_shift))+.01,2)]);
